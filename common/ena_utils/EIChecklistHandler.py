...
from common.dal.copo_da import EIChecklist
...
class ChecklistHandler:
    ...
    def updateCheckList(self):
        ...
        reads = EIChecklist().execute_query({"primary_id": "read", "deleted": get_not_deleted_flag()})
        read = None
        if reads:
            read = reads[0]
        ...
        for checklist in checklist_set:
            ...
            EIChecklist().get_collection_handle().find_one_and_update({"primary_id": checklist["primary_id"]},
                                                                            {"$set": checklist},
                                                                            upsert=True)
            
            write_manifest(checklist)
   
class EICheckListSpreadsheet:
   ...
   def loadManifest(self, m_format):
        ...
        if self.profile_id is not None:
            ...
            try:
                ...
                checklist = EIChecklist().get_collection_handle().find_one({"primary_id": self.checklist_id})
                if checklist:
   ...
   def validate(self):
        ...
        checklist = EIChecklist().get_checklist(self.checklist_id)  
